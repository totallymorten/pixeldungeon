...
public abstract class Entity implements Updateable, Renderable, Collideable
{
	...
	public void render(Graphics2D g, Vector2D viewPos)
	{
		...
		if (rotation != 0)
		{
			...
			if (Game.game.DEBUG)
			{
				...
				collision = getCollisionBox(viewPos);
	...
	public void update(double ms)
	{
		...
		if (setSpeed != null)
		{
			if (!(setSpeed.x != 0 && MathTool.unit(speed.x) == MathTool.unit(setSpeed.x) && Math.abs(speed.x) > Math.abs(setSpeed.x)))
				speed.x += acc.x * secs;
				
			if (!(setSpeed.y != 0 && MathTool.unit(speed.y) == MathTool.unit(setSpeed.y) && Math.abs(speed.y) > Math.abs(setSpeed.y)))
				speed.y += acc.y * secs;
		}
		else
		{
		...
		// scene friction X
		if (acc.x == 0 || MathTool.unit(acc.x) != MathTool.unit(speed.x) || Math.abs(speed.x) > Math.abs(setSpeed.x))
		{
		...
		// scene friction Y
		if (acc.y == 0 || MathTool.unit(acc.y) != MathTool.unit(speed.y) || Math.abs(speed.y) > Math.abs(setSpeed.y))
		{
